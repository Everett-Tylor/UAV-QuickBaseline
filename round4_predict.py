...
def run(a):
    ...
    if a.ensemble_checkpoint:
        other=Segmenter(a.source,config_only=True).cuda().eval()
        ...
        for x in xs:
            ...
            for flip in ([False,True] if a.hflip else [False]):
                for member in models:
                    with torch.autocast('cuda',dtype=torch.float16):logits=member(x.flip(-1) if flip else x)
    ...
    if a.masks:
        report={'checkpoint':a.checkpoint,'ensemble_checkpoint':a.ensemble_checkpoint,'ensemble_weights':'equal','sizes':a.sizes,'hflip':a.hflip,'group_thresholds_from_train':thresholds,
